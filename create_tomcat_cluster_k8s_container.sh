...
buildah umount quay.io/${QUAY_USER}/tomcat${TOMCAT_VERSION}-cluster-k8s-java13
buildah rm quay.io/${QUAY_USER}/tomcat${TOMCAT_VERSION}-cluster-k8s-java13
...
#cluster setting
#
buildah add tomcat${TOMCAT_VERSION}-java13 conf/host_cluster_k8s.xml /tmp/
buildah run tomcat${TOMCAT_VERSION}-java13 /usr/bin/awk -i inplace '/\SingleSignOn/{while(getline line<"/tmp/host_cluster_k8s.xml"){print line}} //' /opt/tomcat9/conf/server.xml
buildah run tomcat${TOMCAT_VERSION}-java13 /usr/bin/rm /tmp/host_cluster_k8s.xml

buildah config  --entrypoint  '["/opt/tomcat9/bin/catalina.sh", "run"]' --workingdir /opt/tomcat9  tomcat${TOMCAT_VERSION}-java13
buildah commit tomcat${TOMCAT_VERSION}-java13 quay.io/${QUAY_USER}/tomcat${TOMCAT_VERSION}-cluster-k8s-java13
